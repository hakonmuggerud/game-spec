# The Undercroft — Design (as implemented)

What the game does, with the numbers read from the code. The game is the Rust/Bevy workspace in this directory
(three crates: `undercroft-data` types and loaders, `undercroft-sim` pure game logic, `undercroft` the Bevy app;
§12 has the layout). **The tables of numbers below are documentation; the source of truth for every number is the
RON under `assets/data/`** — `config.ron` (tuning: player, lamp, hunters, economy, tiers, costs, audio, keys),
`zones.ron` + `maps/<id>.txt` (zones, regions, shortcuts, anchors, the ASCII maps, the hub rows), `palettes.ron`,
`contracts.ron`, `npcs.ron`, `buildings.ron`, `endgame.ron`, `models.ron` (voxel-box models) and
`assets/audio/manifest.ron` (the one-shot clips). Edit those, keep `cargo test` green, and fix the number here
when it changes. The document was written against the Three.js prototype the port was verified against (frozen
under `../reference/prototype/`); where a rule is cited to a Rust module it is `crate::module::item`, and the doc
comment on every ported item still names its JS origin.
...
## 2. Controls (`config.ron` `keys`, matched on the winit key code by its JS `KeyboardEvent.code` name — `undercroft::world::input::js_code`)
...
### 3.2 Zones (`zones.ron` + `assets/data/maps/<id>.txt`; row 0 = north, x = column, z = row)
...
Each zone's named regions, deep pockets and per-region loot split are §3.4 (and `regions` in the zone's `zones.ron` entry);
`ZONES[id].anchors` is the cell contract every test suite reads instead of a literal (§3.7).

#### The Undercroft — 62×62 (`maps/undercroft.txt`)
```
    0         1         2         3         4         5         6 
 ...
 61 ##############################################################
```
#### The Cistern — 64×64 (`maps/cistern.txt`)
```
    0         1         2         3         4         5         6   
 ...
 63 ################################################################
```
#### The Ossuary — 62×62 (`maps/ossuary.txt`)
```
    0         1         2         3         4         5         6 
    01234567890123456789012345678901234567890123456789012345678901
  0 ##############################################################
 ...
 61 ##############################################################
```
#### The Source — 60×60, five laps of gallery + back-spur + lap wall, altar chamber at the centre (`maps/source.txt`)
```
    0         1         2         3         4         5         
...
### 3.3 Scale, cost and expedition length (measured — `undercroft_sim::validate::validate_all`, whose `stats` the fixture `assets/fixtures/validate_all.json` pins)
...
**Expedition length and oil** (measured on the prototype with a throwaway script, not kept — the route walked cell by cell, deep and water cells costed
...
**Render and CPU cost** (measured on the Three.js prototype, headless SwiftShader at 426×240; kept as the budget the Bevy build was written to). Draw calls do
...
hunter counts are frozen (3 / 4 / 4 / 5 records; the creature tests assert them), so pressure comes from siting each
...
- **Data.** `undercroft_data::map::parse_map` sets the cell to `CellKind::Shortcut` and pushes
  `ParsedMap::shortcuts` (`{cx, cz, idx, x, z, open, id}`) row-major, binding each parsed cell to the `shortcuts` entry
  of the zone's `zones.ron` record that lists it and copying its `id`. `grid::is_solid` and `grid::los` both treat a
  barred cell as solid, so the player, hunters, the follower's BFS and every sense check agree with no new rule in
  `undercroft_sim::creature`.
- **Interaction.** `undercroft::player`'s interact resolution runs endgame → npc → hub → items → gates →
  **shortcuts** → stairs; `shortcut_target` mirrors `gate_target` (within `interact_r + 0.5`, facing it). From the
  `open_from` side the hint is `[E] Lift the bars` → `undercroft_sim::world::open_shortcut` rewrites every cell of the
  group to `Floor`, the world lane swaps the barred mesh for the raised one (`world::features::ShortcutDoor`), the id is
  recorded in `save.shortcuts[zone]` and **`SimEvent::ShortcutOpened {zone_id, id, name, cx, cz, idx}`** is emitted
  (toast "The bars fall. <name> is open for good."). From the barred side the hint is `Barred from the other side` and
  `E` only emits `UiError`. The creature module drops its cached paths on `ShortcutOpened` exactly as on `GateOpened`;
  the audio lane answers with the gate cue pitched down plus a chain rattle and a stone boom.
- **Rendering.** The `shortcutBarred` model in `models.ron` is a portcullis — stone lintel, heavy iron bars, a chain drum, and a brass
  lift-bar with a small emissive amber glint **on the far side only**, so the side that can open it is readable through
  the bars; `shortcutOpen` is the same frame with the bars raised into the lintel (walkable underneath), so you
...
One ASCII map per zone under `assets/data/maps/<id>.txt` (one row per line, `size` lines of `size` chars) plus the hub
(`maps/hub.txt`), and one `ZoneDef` record per zone in `zones.ron` (`undercroft_data::zone::ZoneDef`, in `ZONE_ORDER`),
carrying everything that is not the map itself:

```ron
(
  id: "undercroft",                 // matches the map file and ZONE_ORDER
  size: 62,                         // square; the map file has `size` rows of `size` chars
  name, entry, exit, burn_mul, lamp_mul, deep_style, hunters, creatures, npc, npcs, gate, spots, loot, points,
  requires, lock_reason, intro, threat, ambience, hunter_speeds, bands, targets,
  regions:   [(id, name, x: (x0, x1), z: (z0, z1), deep, loot: (oil, relic, rich)), …],
  shortcuts: [(id, name, cells: [(cx, cz), …], open_from: N | E | S | W, from, to, saves), …],
  anchors:   {"entry": (cx, cz), "gate": …, "warden": …, "shortcuts.u_rood": …},
)
```

`palettes.ron` holds the per-zone palettes (`Palette`), `config.ron` `hub_ox` the hub's x offset. `GameData::from_dir`
attaches the rows to each `ZoneDef` at load and parses every map once so a bad edit fails at startup, not in the sim.
`grid::lap_of(cx, cz, m)` reads `m.w/m.h` and `m.bands` (`(band: 5, max_lap: 5)` from the zone's `bands`), falling back
to the legacy 40×40 `(band: 3, max_lap: 5)` when called with no map (`lap_of_legacy`).

**`anchors` is the test contract.** The creature scenarios (`undercroft_sim::creature::tests`) and the app's scripted
runs teleport to anchor lookups rather than to literal grid cells; whoever moves a feature moves its anchor in the same
commit. Beyond the feature cells each zone declares the probe
...
**`undercroft_sim::validate::validate_map` must pass with zero errors for all four zones plus both hubs**
(`validate_all`; the test suite runs it on every `cargo test` and compares the result with the frozen
`assets/fixtures/validate_all.json`, plus negative controls on synthetic maps).
...
lap, every fissure joining L to L+1). `validate_all` returns each zone's `stats` including `shortcuts` and
`route: {closed, open}`, so every number in §3.3 and §3.2 can be read straight off the result.
...
stands, `undercroft_sim::follower::hub_spot`) and the cell(s) west of the anchor stay free: Workshop `1` (3,1) rot π, Wick at (4,1) ·
...
Collision (`undercroft_sim::collision::BlockMask`, the `HubMap.mask` resource, bits `config.ron` `hub_block`): building footprints (ghost or built), the brazier, the residents'
stand cells (rebuilt by the hub lane on hub entry / build / rescue and every 1 s) and the camp props (`undercroft::hub::props`) are solid
...
candle clusters and anything hung from the ceiling stay walkable. `BlockMask::colliders` lists the blocked cells.

Camp (`undercroft::hub::props::HUB_PROPS_V2`, box lists from `models.ron`'s `PROP_BOXES`, merged by the hub lane into one lit + one unlit vertex-colour mesh = 2 draw
...
Light and warmth (hub only; zones keep their palettes): ambient / fog tint per tier `config.ron` `hub_warmth` (ambient 0x1c140d →
0x4c3620, fog 0x050302 → 0x0f0a06 at density 0.10). Hanging lanterns (`undercroft::hub::lights::HUB_LANTERNS_V2`, one merged group per tier, shown
...
11 hub point lights, all despawned with the hub scene while a zone runs. Alcove sconces light up by tier as before: tier 2
the W/E alcoves (1,7) (23,7), tier 3 the N rooms (1,2) (23,2), tier 4 everything (`config.ron` `sconce_int` 1.6 / 3.2 / 5.0; below its tier a sconce keeps a 0.45 banked-ember glow so no room is pitch black). The flame
light flickers on three sines (0.80–1.0 × tier intensity); `config.ron` `embers` (18 / 36 / 60 / 90 alive by tier, `undercroft::hub::flame::Ember` entities) rise from the coals and go out below the ceiling. Residents bob and turn to the player within 5 u
and idle in place (head turn, breathing, arm sway; `undercroft::hub::npcs`). The v1 17×9 hub rows are kept in the
frozen fixture `assets/fixtures/hub_v1.json` for tests only.

QA (the hub audit, run on the prototype; the Bevy build inherits the layout unchanged): every hub cell and collider dumped with everything built / everyone rescued,
checking that every doorway is free with free cells on both sides, that every free cell is reachable from the spawn with
...
## 5. Hunters and creatures (`undercroft_sim::creature`)
...
light play (douse + pools + flash), never only by outrunning. Numbers are relative to `config.ron`: walk **2.6** / sprint
...
**Touch (0.9 u, target not in a pool)** no death. Emits `LampSnuffed {hunter_id, oil: 12, lockout: 2, x, z}`; `undercroft_sim::economy::on_lamp_snuffed`:
...
audio thuds it (≤ 26 u); `undercroft::world::camera::ScreenFx::shake` shakes the camera when d ≤ 8: pitch `0.012·(1 − d/8)·sin(28t)` decaying over 0.25 s,
...
### 5.6 Tuning table (`config.ron` `hunter_profiles` + the `creature` block)
...
### 5.7 Placement
New legend characters (`undercroft_data::cell::legend`): `L` lampwight · `G` warden (guard; `W` is already water) ·
...
z30 #..#.#DD#D#DDLD#DD#D#DD#DDDDDDADDD#DDDD#DD#D#D#DD##DDD#o...#      the last room before the chamber door (25,30) — lanterns are the
                                                            ...
                                                            Source dormancy still wakes each at lap - 1.
```
...
### 5.8 Architecture — one driver, a profile table
One list (`Zone.hunters`, `Vec<Hunter>`), one record shape (`creature::record::Hunter`), one
`driver::update` / catch path; behaviour comes from a **profile table**. `creature::spawn::spawn_all` reads
`ParsedMap::hunter_spawns` (H, profile from the zone's `hunters`) then `ParsedMap::creatures` (kind = profile, options
from the zone's `creatures`); every record gets `home` (spawn cell), `opts`, and its `ProfileKind`. The profile
(`creature::profiles::ProfileDef`, one per kind, numbers read from `config.ron` by `Tuning::from_config`):

```rust
pub struct ProfileDef {
    kind: ProfileKind, model: &'static str,          // the models.ron entry: hunter | lampwight | warden | drowner | falseLight | brute
    initial: HState,                                 // Wander | Sentry | Submerged | Lit
    speed: [f32; HState::COUNT], eye: [Option<f32>; HState::COUNT], catch_r, lose_t, scale_y, eye_color,
    kills: bool,                                     // false = the catch snuffs the lamp (Lampwight) instead of killing
    senses: Senses,                                  // lamp, sprint, walk, still, water, cone (deg, reach), near 1.2, proximity 3.0, follower
    creature: bool, pool_mul: 1.0 | 0.5, water_mul: 0.85 | 1.0, catch_in_pool: bool,
    leash: Option<i32>, territory: Option<f32>, step: Option<[f32; HState::COUNT]>,
    fsm: Fsm,                                        // row(HState) -> StateRow { mv: Path | Still | Lunge, repath, close, catch, sweep, … } + tick
    hooks: Hooks,                                    // sense, on_flash, can_enter, can_enter_loose, on_reset, on_tick, on_near_lantern, catch_if, on_catch, anim
}
```
- **Driver** (`creature::driver`): per record `update_one` → the 0.2 s `tick`: decay `daze_t/busy_t` → `stim = hooks.sense`
  (`senses::generic_sense` over `Senses`; cone / near / proximity are extra predicates) → `fsm.tick(ctx, h, stim)`;
  `set_state` runs the enter hook. The path-follow / direct-close / catch code is shared and reads `can_enter`,
  `pool_mul`, `water_mul`, the row's `catch` flag, `catch_in_pool` and `mv` (`Still`: Warden SENTRY, False light
  LIT/RELIGHT · `Lunge`: straight line, no path). BFS only on repath ticks and wander legs, never per frame.
- **Base FSM as data** (`creature::base`): WANDER / INVESTIGATE / CHASE / STAGGERED for `base`/`fast`; `brute`
  (`creature::brute`) = it minus STAGGERED plus `leash`/`on_near_lantern`/`step`; `lampwight` (`creature::lampwight`)
  = it with DRIFT/DRAWN plus SNUFF/SATED; Warden, Drowner, False light are their own 5–6-state tables
  (`creature::warden`, `creature::drowner`, `creature::false_light`). `grid::bfs_field` takes the blocked predicate.
- **Flash / catch** `undercroft_sim::economy::flash` finds the target (cone/LOS test, `creature::driver::flash_targets`) and calls
  `hooks.on_flash`. The driver emits `HunterCatch` only for a killing profile; a Lampwight catch emits `LampSnuffed`
  and the economy applies `lamp_lock`. `DebugCommand::SpawnCreature` is the test hook. **Endgame** dormancy loops the
  records by spawn lap; the active-count and `economy::pick_spawn_cell` filters use `base`/`fast` only. **Audio**
  (`undercroft::audio::voices`) is per-profile presence voices (growl · whistle · grind · plops · breath; false light
  none); the one-shots hang off the events. **Minimap / pools / follower** read only the lantern list and the
  follower's stimulus.

**Events reused** (`SimEvent` variants; the JS names in the doc comments) `ZoneEnter`, `ZoneExit`, `HubEnter`,
`Lantern`, `LanternRemoved` (also fired by a smash → pools), `GateOpened`, `NpcCaught`, `Flash`, `WaterEnter/Exit`,
`HunterState {id, state, prev}` (the HUD's "It has seen you" keys on CHASE/POUNCE), `HunterCatch {x, z, hunter_id,
target}` (death), `HunterWoken`, `HunterSpawned`.
**Creature events** `LampSnuffed {hunter_id, oil, lockout, x, z}` · `LanternSmashed {hunter_id, x, z}` · `WardenAlert
{hunter_id, x, z}` · `WardenReturn {hunter_id}` · `DrownerSurge {hunter_id, x, z}` · `DrownerSink {hunter_id}` ·
`FalseLightPounce {hunter_id, x, z}` · `FalseLightReveal {hunter_id, x, z}` · `FlashResisted {hunter_id, profile}` ·
`CreatureStep {hunter_id, profile, x, z, d}`. All payloads carry `hunter_id` so audio pans and tests filter by profile.

### 5.9 Acceptance checklist (headless; `undercroft_sim::creature::tests` covers the per-creature rules on synthetic maps)
Harness: `DebugCommand::{GotoZone, Teleport, SpawnCreature, Key}` through `headless_app()` (`undercroft::headless`), the `EventLog`, and `step(app, secs)` until `tick::Clock` reaches T; the same steps run in the real app as `UNDERCROFT_SCRIPT`.
- [ ] **Maps** `validate_all().ok`; each zone's `ParsedMap::creatures` matches §5.7 cells/kinds; `(20,13)` is `Water`;
      `spawn_all` gives Undercroft 3, Cistern 4, Ossuary 4, Source 5 records; Source `L Y B` start inactive (dormant).
...
- [ ] **Cross-cutting** every creature model in `models.ron` has ≤ 30 boxes and is named by its profile's `model`;
      ...
      the headless integration tests still pass and the log stays clean across all four zones for 60 s of wandering.

### 5.10 Balance pass (measured on the prototype with throwaway headless scripts; the numbers are what the port reproduces)
...
## 7. NPCs (`undercroft_sim::follower`, `npcs.ron`) and contracts (`undercroft_sim::contracts`, `contracts.ron`)
...
## 8. Hub buildings and services (`undercroft_sim::economy`, `buildings.ron`; rendered by `undercroft::hub`)
...
## 9. Endgame (`undercroft_sim::economy` Source run / altar / endings, `endgame.ron`)
...
## 10. Audio (`undercroft::audio`: `synth` for the continuous layers, `oneshots` for the baked clips in `assets/audio/`, `voices` per creature)
...
## 11. Save (`undercroft_sim::save::SaveData`; `./undercroft-save.json` natively, `localStorage` key `undercroft-v2` on the web)

Written debounced 250 ms (`undercroft::run::SAVE_DEBOUNCE`) on every event. The JSON form is byte-compatible with the
prototype's, so a browser save from it loads here. Imports v1 `undercroft-proto` points/oil once. Unknown keys are ignored,
wrong types fall back to defaults (`save::merge_into`).
```js
...
```
**v2 → v3 migration** (`save::migrate_v2`; §3.6, additive): a v2 record keeps points/oil/relics/tools/buildings/rescued/
...
| File | Owns |
|---|---|
| `lib.rs` | `SkeletonPlugin` (states, resources, messages, debug queue, the fixed tick — everything without a window; added by the app *and* the headless harness) and `UndercroftPlugin` (skeleton + the asset loader + the five lanes) |
| `state.rs` | `GameMode` (`Loading → Title → Hub ↔ Zone → Dying → Dead → Hub`, plus `Menu` and `Ending`), `PrevMode`, `MenuKind`, the `Mode` system param (read this inside `FixedUpdate`: the pending state wins, as `ctx.state.mode` did within a JS tick) |
| `resources.rs` | every shared resource: `Player`, `Zone` (`hunters`, map, items, lanterns), `HubMap` (map + `BlockMask`), `HubRes`, `LampRes`, `Npcs`, `SaveRes`, `RngRes`, `Fade`, `Toasts`, `MoveIntent` … — the sim's types held once, written by `run.rs` / `player.rs` |
| `messages.rs` | `SimMessage(SimEvent)`, the Bevy message every sim call's `Vec<SimEvent>` is fanned out as; `EventLog`, a 4096-entry ring buffer of `(tick, event)` that tests and the debug script read |
| `tick.rs` | the 60 Hz `FixedUpdate` and `SimSet` order: `Debug → Input → Player → Creatures → Follower → Contracts → Economy → Fanout` (input → player → world → hunter → npc → contracts → hub/endgame → listeners, as the JS frame ran); `TickCount` |
| `debug.rs` | `DebugCommand` — the replacement for `window.__game.actions`: one variant per state-changing action (`Begin`, `Descend`, `Bank`, `Flash`, `PlantLantern`, `Interact`, `TopUp`, `ToggleLamp`, `OpenMenu`, `LoadZone`, `SelectZone`, `FreeNpc`, `Accept`, `Build`, `Choose`, `GiveTool`, `SetPoints`, `SetResources`, `UnlockAll`, `GotoZone`, `Teleport`, `SpawnHunter`, `SpawnCreature`, `RideUp`, `Reset`, `Key(code)` …), queued in `DebugQueue`, drained first in the tick; `DebugCommand::parse` reads the JS names (`begin`, `gotoZone undercroft`, `teleport x z yaw`, `key KeyE`) plus `wait N`, `screenshot path`, `quit`, which is the `UNDERCROFT_SCRIPT` grammar |
| `run.rs` | the game lifecycle: every mode change, the fade, hub / zone / bank / death loop, menus, the Source run and the endings, the save writer (`SAVE_DEBOUNCE`), and the per-tick sim calls that are not the player's (creatures, follower, contracts, economy) |
| `player.rs` | movement, the handlamp, flash, planted lanterns, pickups, the interaction target and the in-run key handler; feeds the sim the frame's inputs and emits what it returns |
| `assets.rs` | `GameDataAsset` and its loader: `assets/data/game.gamedata.ron` is the root, the eight RON tables and five maps are its dependencies, so they hot-reload natively under the `dev` feature |
| `model.rs` | a `ModelDef` from `models.ron` (a voxel-box factory recorded from the prototype) → an entity hierarchy: one lit + one emissive mesh per model |
| `headless.rs` | `headless_app()`: `MinimalPlugins` + the skeleton with `assets/data/` loaded synchronously, an in-memory save store; the test harness (`tests/skeleton.rs` shows `send` / `step` / `log` / `mode`) |
| `world/` | camera, keyboard / mouse / pointer lock (`input`), the instanced map blocks (`blocks`), palettes and exposure (`palette`, `EXPOSURE_EV100`), per-zone atmosphere and lap tint, gates and shortcut doors (`features`), water, items, planted lanterns, screen shake and the death camera. Writes `MoveIntent` |
| `creatures/` | the visual side of every creature: mirrors `Zone.hunters` onto entities by id (`sync`), eye glow, animation, the burst effects |
| `ui/` | HUD, hint, toasts, the fade, every screen and menu (`screens`, `menu`, keyboard and mouse routing), the minimap, the palette and type scale (`style`, `assets/ui/` font). Writes `Toasts` |
| `hub/` | the hub scene: the great flame and its tiers, embers, sconces and hanging lanterns (`lights`), buildings and ghosts, the camp props and their collision footprints (`props`, writes `HubMap.mask`), resident / captive / follower meshes and idle animation (`npcs`) |
| `audio/` | `synth` (ambience, drips, lamp crackle, flame — the continuous layers as a generated audio source), `oneshots` (the 59 clips in `assets/audio/` by `manifest.ron`, panned per event), `voices` (one presence voice per creature record, by profile). Owns `save.audio` |

The lanes only *read* game state and push `DebugCommand`s to change it; `run.rs` and `player.rs` are the writers
(exceptions on record: world writes `MoveIntent`, hub writes `HubMap.mask`, audio owns `save.audio`, ui writes
`Toasts`). Lanes need a window and a renderer, so they stay out of the headless harness. The app never uses
`std::fs` / `std::thread` / `Instant` outside `cfg(not(wasm32))`; the save is `./undercroft-save.json` natively
(`UNDERCROFT_SAVE` overrides) and `localStorage` on the web.

**Events.** `undercroft_sim::events::SimEvent` is the whole vocabulary — one variant per name the prototype's bus
carried, with the JS payload fields, and `SimEvent::name()` returns the JS name so a log can be compared with the
prototype's traces: `Begin`, `HubEnter`, `ZoneEnter`, `ZoneExit`, `Pickup`, `Bank`, `FlameTier`, `Death`, `HunterCatch`,
`HunterState`, `Flash`, `Lantern`, `LanternRemoved`, `GateOpened`, `ShortcutOpened`, `NpcFreed`, `NpcCaught`,
`NpcRescued`, `ContractAccepted`, `ContractComplete`, `ContractFailed`, `ToolGained`, `Build`, `LightTech`, `Service`,
`ZoneSelected`, `Blessing`, `Lap`, `Ending`, `UiClick`, `UiError`, `Toast`, the menu / key events, plus the creature
events of §5.8. Every sim call returns `Vec<SimEvent>`; the shell writes each as a `SimMessage`, mirrors it into
`EventLog` in `SimSet::Fanout`, and the lanes react by reading `MessageReader<SimMessage>` the way the JS listeners did.
Randomness goes through `SimRng` only, so a run is seedable and every test deterministic.
