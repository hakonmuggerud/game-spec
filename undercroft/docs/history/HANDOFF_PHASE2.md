> Historical. This is `undercroft/HANDOFF.md` as it stood when Phase 2 was planned (commit 3cfab5b,
> 2026-09-09); doc comments in the code that cite "HANDOFF §6" (the Phase 2 plan and its open items) or
> "HANDOFF §8" (conventions, the single RNG stream) mean this file. Paths in it predate the move of the
> prototype to `reference/` and of these documents to `docs/history/`.
...
| 2 | Bevy shell (world, creatures, UI, hub, audio plugins) | **Skeleton done 2026-09-08** (`PHASE2_SKELETON.md`): states, data asset loader, shared resources, `SimMessage` bus + `EventLog`, `DebugCommand`s, 60 Hz tick, the full `main.js` lifecycle (`run.rs`) and player (`player.rs`), headless harness, 44 app tests. The five rendering/UI lanes in §6 have not started. |
...
undercroft/         this workspace
  ...
  crates/undercroft        the Bevy app: skeleton (see PHASE2_SKELETON.md §1 for the file map);
                           the Title screen is still the Phase 0 cube until the ui lane lands
...
The **skeleton step is done**; `PHASE2_SKELETON.md` is the contract the lanes build on (its §8 lists
what changed from the first draft). What the lanes get: `GameMode` states with a `state::Mode`
system param (pending state wins; always use it inside `FixedUpdate`), `Game` for the data,
`ZoneRes`/`HubMapRes`/`Npcs`/`Player`/`LampRes`/`SaveRes` as the only game state, `MoveIntent` to
fill from input (raw pointer deltas; `CFG.mouseSens` is applied in `player.rs`), `SimMessage` to read
every `SimEvent` in JS order, `DebugCommand`s for anything a menu or key needs to trigger, and the
headless harness (`cargo test -p undercroft --features dev`, 44 tests) for parity checks.

Open items the lanes inherit (from the verification pass):

- world: fill `SimSet::Input` → `MoveIntent`; keyboard look (`CFG.lookKeys × dt`) must be pre-divided
  by `mouse_sens`; death camera (`main.js:383–394`); `fx.shake` on `creatureStep`; rebuild
  `HubMapRes`/`ZoneRes.map` on `AssetEvent::Modified` if map hot reload is wanted (config already
  hot-reloads through `GameDataAsset`).
- ui: flush toasts on `title` (the "Left below" toast is emitted after `title`, as in the JS);
  title-menu keys incl. the Backspace×2 wipe; route pause keys through the menu instead of the
  direct `ClosePause`/`CloseMenu` forwarding in `player.rs::on_key`; minimap from the save bitset;
  the wasm `pagehide` save flush (native `AppExit` flush exists in `run.rs`).
- hub: prop footprints into `HubMap.mask`; the night-visit tier-1 flame; board/build/service/dialog
  panels behind the `OpenMenu(kind)` commands `player.rs` already pushes.
- creatures: `Zone.hunters` is authoritative, indexed by `Hunter.id`; nothing blocking.
- audio: `lap` cue and `economy::lap_tint` have no consumer yet.
- known one-tick staleness: a Brute lantern smash is applied after the whole `creature::update`
  batch, so other hunters in that tick still see the lantern (the JS removed it inline).

Then five lanes in parallel, one Bevy plugin each, each in its own git worktree with its own files
(this is how Phase 1 and the skeleton were run; use `CARGO_TARGET_DIR` pointing at the main tree's
`target/` so Bevy is not rebuilt per worktree).
